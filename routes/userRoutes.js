...
authRoutes.put("/:id", function (req, res) {
    console.log("in put");
    User.findOneAndUpdate({_id: req.body.id}, req.body, function (err, existingUser) {
        if (spell === null) res.status(500).send("No user found");
        if (err) return res.status(500).send(err);
